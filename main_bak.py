...
def downloadImage(url, content, file_saver):
    ...
    for img in imgs:
        logging.info(img);
        # 图片延迟加载的情况，这个时候除了有src属性还会有original属性
        if img.has_attr('src') and img.has_attr('original'):
            ...
            if matches is not None:
                logging.info('Image: Matched outer link.');
                imgUrl = img['original'];
                imgPath = './images/' + matches.group(2) + matches.group(3);
                match_flag = 1;
            else:
                # original指向的是本地服务器图片
                matches = re.match((r'^/(.*\.)(%s)' % config.configs['picture_formats']) , img['original']);
                if matches is not None:
                    logging.INFO('Image: Matched local resouce');
            ...
            if match_flag == 1:
                ...
                # 将HTML中延迟加载的部分改成直接加载
                content = content.replace((r'src="%s" original="%s"' % (img['src'], img['original'])).encode('utf-8'), (r'src="%s"' % imgPath).encode('utf-8'));
        # 普通图片
        elif img.has_attr('src'):
            logging.info('Normal Image');
            match_flag = 0;
            matches = re.match((r'^(http|https)://(.*\.)(%s)' % config.configs['picture_formats']), str(img['src']));
            # 图片是外部链接
            if matches is not None:
                ...
                matches = re.match((r'^/(.*\.)(%s)' % config.configs['picture_formats']), img['src']);
                # 图片是本站资源
                if matches is not None:
